...
- [ ] Expose `beat_confidence` (0.0 to 1.0) for softer reactions.

### Step 2.2: Reactive Effects
- [ ] Create `effects/pulse.py`: An effect that flashes the background color when `is_beat` is true.
- [ ] Create `effects/glitch.py`: An effect that randomly shifts characters or colors in the buffer when a strong bass hit occurs.
- [ ] Add configuration options to `config.py` to enable/disable these new effects.

### Step 2.3: New Themes
- [ ] Add "Cyberpunk 2077" (Yellow/Black/Cyan) to `THEMES`.
- [ ] Add "Matrix Rain" (Black background, multiple shades of Green) to `THEMES`.
- [ ] Add "Synthwave" (Purple/Pink/Orange) to `THEMES`.
